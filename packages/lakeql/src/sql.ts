...
function mapAstSources(
  ...
  if (ast.correlatedScalarSubqueries !== undefined) {
...
function collectSourceColumnsFromAst(
  ...
  for (const subquery of Object.values(ast.correlatedScalarSubqueries ?? {})) {
    ...
    for (const aggregate of Object.values(subquery.aggregates)) {
      if (aggregate.column !== undefined) addColumn(columns, subquery.source, aggregate.column);
...
function collectExprColumnsForSource(
  expr: Expr,
  columns: Map<string, Set<string>>,
  source: string,
): void {
...
interface CorrelatedScalarLookup {
  subquery: NonNullable<SqlQueryAst["correlatedScalarSubqueries"]>[string];
  rows: Map<string, Row>;
}

async function correlatedScalarRowsFromAst(lake: ParquetLake, ast: SqlQueryAst): Promise<Row[]> {
  let rows = await lake.path(ast.source).toArray();
...
function hasCorrelatedScalarSubqueries(ast: SqlQueryAst): boolean {
  return (
...
async function projectRows(lake: ParquetLake, rows: Row[], ast: SqlQueryAst): Promise<Row[]> {
