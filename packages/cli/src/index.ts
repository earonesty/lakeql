...
async function aggregateSubqueryJoinRowsFromAst(
  lake: ReturnType<typeof createParquetLake>,
  ast: ReturnType<typeof parseSql>,
  joins: NonNullable<ReturnType<typeof parseSql>["subqueryJoins"]>,
  args: ParsedArgs,
): Promise<Row[]> {
  let rows = await lake.path(ast.source).toArray();
...
function subqueryJoinAggregateAst(ast: ReturnType<typeof parseSql>): ReturnType<typeof parseSql> {
  const {
...
function emptyAggregateRowsFromAst(ast: ReturnType<typeof parseSql>): Row[] {
