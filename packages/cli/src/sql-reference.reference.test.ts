...
describeReference("SQL CLI DuckDB reference comparisons", () => {
  it.each([
    ...
    {
      name: "grouped IN subquery semi join",
      lakeql:
        "select distinct region from input where region in (select region from input group by region having count(*) > 20 and max(amount) > 900) order by region asc",
  ...
  it("matches DuckDB for correlated predicate IN subquery semi join", async () => {
    const storesPath = await storesFixturePath();
    const lakeql =
      "select store_id, region, amount from sales s where s.store_id in (select d.store_id from stores d where d.region <> s.region) order by amount asc limit 3";
    const duckdb = `select store_id, region, amount from read_parquet('${sqlString(
      fixturePath(SALES.file),
    )}') s where s.store_id in (select d.store_id from read_parquet('${sqlString(
      storesPath,
    )}') d where d.region <> s.region) order by amount asc limit 3`;
  ...
  it("matches DuckDB for multiple correlated grouped EXISTS subqueries", async () => {
    const lakeql =
      "select distinct region from sales s where exists (select 1 from sales x where x.region = s.region group by x.store_id having count(*) > 5) and not exists (select 1 from sales y where y.region = s.region group by y.store_id having count(*) > 100) order by region asc";
    const duckdb = `select distinct region from read_parquet('${sqlString(
      fixturePath(SALES.file),
    )}') s where exists (select 1 from read_parquet('${sqlString(
      fixturePath(SALES.file),
    )}') x where x.region = s.region group by x.store_id having count(*) > 5) and not exists (select 1 from read_parquet('${sqlString(
      fixturePath(SALES.file),
    )}') y where y.region = s.region group by y.store_id having count(*) > 100) order by region asc`;
  ...
  it("matches DuckDB for grouped correlated scalar aggregate predicates with HAVING", async () => {
    const lakeql =
      "select store_id, region, amount from sales s where amount > (select avg(x.amount) as avg_amount from sales x where x.region = s.region group by x.region having count(*) > 5) order by amount asc limit 5";
    const duckdb = `select store_id, region, amount from read_parquet('${sqlString(
      fixturePath(SALES.file),
    )}') s where amount > (select avg(x.amount) as avg_amount from read_parquet('${sqlString(
      fixturePath(SALES.file),
    )}') x where x.region = s.region group by x.region having count(*) > 5) order by amount asc limit 5`;
